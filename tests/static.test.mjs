...
test('interactive deck supports one-card draw, random three-card draw, shuffle, and flip reveal', async () => {
  ...
  for (const id of [
    'visual-draw-stage', 'tarot-deck', 'draw-one-card', 'draw-three-cards',
    'shuffle-deck', 'draw-slot-1', 'draw-slot-2', 'draw-slot-3',
  ]) {
    assert.match(html, new RegExp(`id=["']${id}["']`));
  }
  ...
  assert.match(visualDraw, /shuffleVisualDeck/);
  assert.match(visualDraw, /syncVisualDrawToSelectors/);
  assert.match(visualDraw, /visual-draw\.css/);
  assert.match(visualCss, /transform-style:\s*preserve-3d/);
  assert.match(visualCss, /rotateY\(180deg\)/);
